...
class RecommendationController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            return response()->json(Recommendation::all()->load(['animal', 'visit']));
        }
        $owner_id = Owner::where('user_id', Auth::user()->id)->pluck('id');
        $arrayAnimals = Animal::where('owner_id', $owner_id)->pluck('id')->toArray();
        $recommendation = Recommendation::whereIn('animal_id', $arrayAnimals)->get()->load(['animal', 'visit']);
        return response()->json($recommendation);

    }
    public function show(Recommendation $recommendation)
    {
        return response()->json($recommendation->load(['animal', 'visit']));
    }
